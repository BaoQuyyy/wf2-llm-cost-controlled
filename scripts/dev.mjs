/**
 * One command to bring the WF2 stack up and prove it works.
 *
...
const SERVICES_PORT = 4100
const N8N_PORT = 5678
const WORKFLOW_ID = 'wf2LlmCostCtrl0'
...
async function startServices() {
  ...
  if (!live) die('mock services did not come up on port 4100', readLog('mock-services'))
...
function buildAndImport() {
  ...
  for (const f of ['01-llm-grading.json']) {
  ...
  run(`n8n publish:workflow --id=${WORKFLOW_ID}`)
...
async function startN8n() {
  ...
  const bound = await waitFor(
    ...
    async () => {
      try {
        const res = await fetch(`http://localhost:${N8N_PORT}/webhook/grade`, {
          method: 'POST',
          headers: { 'content-type': 'application/json', connection: 'close' },
          body: JSON.stringify({ document_id: '_probe', text: 'x' })
        })
  ...
  if (!bound) {
    die(
      'n8n is up but /webhook/grade still answers 404.\n' +
...
switch (command) {
  ...
  case 'start': {
    ...
    console.log(dim(`  webhook       http://localhost:${N8N_PORT}/webhook/grade`))
